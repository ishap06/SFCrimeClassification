...
dfTrain = pd.read_csv('./NEW/train/train.csv')
dfTest = pd.read_csv('./NEW/test/test.csv')

#----------------------------------------------------------------------------------
colsToDrop1 = ['Category_New','season_Spring','season_Fall','Year','X', 'IncidntNum']
colsToDrop2 = ['Category_New','Category','Location','PdId','Day', 'IncidntNum','TimeOfDay']
# ---------------------------------------------TRAIN--------------------------
dfTrain = pd.get_dummies(dfTrain, columns=["Season","PdDistrict","DayOfWeek"], prefix=["season","pdDist","day"])
dfTest = pd.get_dummies(dfTest, columns=["Season","PdDistrict","DayOfWeek"], prefix=["season","pdDist","day"])
